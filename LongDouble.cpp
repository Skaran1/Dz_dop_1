...
void LongDouble::setSign(int x)
{
	sign = x;
}

void LongDouble::setExponent(int x)
{
	exponent = x;
}

void LongDouble::setDigits(int* x, int size)
{
	digits.resize(size);
	memcpy(digits.data(), x, size);
}

int LongDouble::getSign() const {
	return sign;
}

long LongDouble::getExponent() const {
	return exponent;
}

std::vector<int> LongDouble::getDigits() const {
	return digits;
}
